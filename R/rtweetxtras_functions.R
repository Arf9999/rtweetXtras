...
rehydrate_got3_statuses <- function(got3_df,token = NULL, token_list = NULL ) {
  ...
  retweet_temp <-purrr::map_df(retweets$status_id, rtweet::get_retweets, n = 100,
                               token = token_list[[row_of_max_rl]])
  message(paste0("temp rows:",nrow(retweet_temp)))

  message("Captured ", nrow(retweet_temp), " retweets (a maximum of 100 per original tweet)")
...
got_search <- function(search_string, since_date, until_date, n = 100, output_file_name, token = NULL, token_list = NULL){
  require(rtweet, quietly = TRUE)
  require(readr, quietly = TRUE)
  require(dplyr, quietly = TRUE)

  output_path <- paste0(output_file_name, Sys.Date(), ".csv")
  system(paste0("GetOldTweets3 --querysearch \"", search_string,
                "\" --since ", since_date,
                " --until ", until_date,
                " --maxtweets ", n,
                " --output \"", output_path, "\""))
  ...
  output <- rtweetXtras::rehydrate_got3_statuses(got_file, token = token, token_list = token_list)
